...
/*
 * Uio_Fpga 类：在通用 Uio 之上封装的 FPGA 专用驱动。
 *
...
typedef struct Uio_Fpga_s Uio_Fpga;

struct Uio_Fpga_s {
    Uio parent;

    int (*construct)(Uio_Fpga *,char *);
    int (*deconstruct)(Uio_Fpga *);

    /*virtual methods reimplement*/
    int (*set)(Uio_Fpga *module, char *attrib, void *value);
    void *(*get)(Uio_Fpga *, char *attrib);
    char *(*to_json)(Uio_Fpga *); 

    /* FPGA interface (built on top of Uio)
     * open_device(dev_path) 直接按 /dev/uioX 路径打开 FPGA UIO 设备并 mmap。
