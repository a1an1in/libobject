...
typedef struct My_ID3v2ExtraMeta {
    const char *tag;
    void *data;
    struct My_ID3v2ExtraMeta *next;
} My_ID3v2ExtraMeta;
...
typedef struct  pls_key_value_t {
    int bandwidth;
...
struct My_segment {
    ...
    char *url;
    char *key;
    enum My_KeyType key_type;
    uint8_t iv[16];
    /* associated Media Initialization Section, treated as a segment */
    struct My_segment *init_section;
};

struct My_rendition;

enum My_PlaylistType {
...
struct My_playlist {
    ...
    int finished;
    enum My_PlaylistType type;
    int64_t target_duration;
    int start_seq_no;
    int n_segments;

    struct My_segment **segments;
    int needed;
    ...
    struct My_segment *cur_init_section;
    ...
    My_ID3v2ExtraMeta *id3_deferred_extra; /* stored here until subdemuxer is opened */
    ...
    int n_renditions;
    struct My_rendition **renditions;

    /* Media Initialization Sections (EXT-X-MAP) associated with this
     * playlist, if any. */
    int n_init_sections;
    struct My_segment **init_sections;
};
...
typedef struct My_HLSContext {
    AVClass *class;
    AVFormatContext *ctx;
    int n_variants;
    struct My_variant **variants;
    int n_playlists;
    struct My_playlist **playlists;
    int n_renditions;
    struct My_rendition **renditions;
...
void read_seek(AVFormatContext *s, int stream_index,int64_t timestamp, int flag) {
                                   hls_read_seek(s,seek_stream_index,timestamp,flag);
}
#endif 