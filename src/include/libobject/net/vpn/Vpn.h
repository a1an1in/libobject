...
typedef struct vpn_cfg_s {
     ...
     * 链路建立后自动通告给对端（被动方还会在应答里回带自己的），对端据此自动
     * `ip route replace <它> dev tun`。**对端网段一律来自运行时交换，没有静态配置入口。**
     * 可空=不通告（此时只有隧道内地址互通）。 */
 ...
 * 双向转发（出站 tun read -> 按目的地址选链路 send；入站 recv 回调 -> tun write），
 * 阻塞直到 Ctrl+C(SIGINT) 或出错。
 *
 * 多对端（一个 tun 复用给多条链路）：
 *   - peer_id 非空 = 主叫，主动连这一个对端；
 *   - peer_id 为空 = 被叫，可同时被多个主叫连接（上限见实现里的 VPN_MAX_LINKS），
 *     被叫无需事先知道对端是谁；对端隧道地址/内网网段由链路建立后自动互换。
 * 出站选路：对端隧道地址（精确）> 对端内网网段（最长前缀）；都不命中且只有一条链路
 * 则退化为直发（点对点兼容），多条链路时丢弃。
 *
 * 返回：0=正常停止；负值=失败。
 */
