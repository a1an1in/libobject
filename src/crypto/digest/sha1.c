...
digest_sha1_body(digest_sha1_t *ctx, const uint8_t *data, unsigned int size)
{
...
digest_sha1_update(digest_sha1_t *ctx, const void *data, unsigned int size)
{
    ...
    if (used) {
        ...
        size -= free;
        (void) digest_sha1_body(ctx, ctx->buffer, 64);
    }

    if (size >= 64) {
        data = digest_sha1_body(ctx, data, size & ~(unsigned int) 0x3f);
        size &= 0x3f;
    }
...
static int
test_sha1(TEST_ENTRY *entry, void *argc, void *argv)
{
    ...
    digest_sha1_update(&ctx, test, strlen(test));
    digest_sha1_final(&ctx, result);
