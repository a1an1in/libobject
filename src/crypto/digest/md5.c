...
digest_md5_body(digest_md5_t *ctx, const uint8_t *data, unsigned int size)
{
    uint32_t       a, b, c, d;
...
void 
digest_md5_update(digest_md5_t *ctx,
                  const void *data, unsigned int size)
{
    unsigned int  used, free;

    used = (unsigned int) (ctx->bytes & 0x3f);
    ctx->bytes += size;
    ...
    if (used) {
        ...
        size -= free;
        (void) digest_md5_body(ctx, ctx->buffer, 64);
    }

    if (size >= 64) {
        data = digest_md5_body(ctx, data, size & ~(unsigned int) 0x3f);
        size &= 0x3f;
    }
...
static int
test_md5(TEST_ENTRY *entry, void *argc, void *argv)
{
    char *expect = "25f9e794323b453885f5181f1b624d0b";
    unsigned short ret;
    digest_md5_t ctx;
    char *test = "123456789";
