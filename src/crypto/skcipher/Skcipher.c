...
test_skcipher_ecb_aes(TEST_ENTRY *entry, void *argc, void *argv)
{
    ...
    TRY {
        ...
        EXEC(base64_encode(out, strlen(out), out_base64, 1024));
        dbg_str(DBG_DETAIL, "base64 encode:%s", out_base64);

        EXEC(base64_decode(out_base64, strlen(out_base64), out_base64, 1024));
        dbg_buf(DBG_DETAIL, "base64 decode:", out_base64, strlen(out_base64));
        
        EXEC(sk->decrypt(sk, out, strlen(in), out, strlen(in)));
