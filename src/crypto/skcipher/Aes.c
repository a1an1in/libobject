...
//refer: https://blog.csdn.net/weixin_56375405/article/details/123896872

static volatile const u8 aes_sbox[] = {
...
static volatile const u8 aes_inv_sbox[] = {
...
static int __construct(Aes *aes, char *init_str)
{
    return 0;
}

static int __deconstruct(Aes *aes)
{
    return 0;
}
...
static int __set_key(Aes *ctx, char *in_key, unsigned int key_len)
{
	...
	for (i = 0; i < kwords; i++)
		ctx->key_enc[i] = get_unaligned_u32(in_key + i * sizeof(u32));
...
static int __encrypt(Aes *ctx, const u8 *in, u8 *out)
{
	...
	st0[0] = ctx->key_enc[0] ^ get_unaligned_u32(in);
	st0[1] = ctx->key_enc[1] ^ get_unaligned_u32(in + 4);
	st0[2] = ctx->key_enc[2] ^ get_unaligned_u32(in + 8);
	st0[3] = ctx->key_enc[3] ^ get_unaligned_u32(in + 12);
	...
	put_unaligned_u32(subshift(st1, 0) ^ rkp[4], out);
...
static int __decrypt(Aes *ctx, const u8 *in, u8 *out)
{
	...
	st0[0] = ctx->key_dec[0] ^ get_unaligned_u32(in);
	st0[1] = ctx->key_dec[1] ^ get_unaligned_u32(in + 4);
	st0[2] = ctx->key_dec[2] ^ get_unaligned_u32(in + 8);
	st0[3] = ctx->key_dec[3] ^ get_unaligned_u32(in + 12);
	...
	put_unaligned_u32(inv_subshift(st1, 0) ^ rkp[4], out);
	put_unaligned_u32(inv_subshift(st1, 1) ^ rkp[5], out + 4);
	put_unaligned_u32(inv_subshift(st1, 2) ^ rkp[6], out + 8);
	put_unaligned_u32(inv_subshift(st1, 3) ^ rkp[7], out + 12);

    return 0;
}

static class_info_entry_t Aes_class_info[] = {
    Init_Obj___Entry(0, Skcipher, parent),
