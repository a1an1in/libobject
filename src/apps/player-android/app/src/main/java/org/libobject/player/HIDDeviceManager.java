...
public class HIDDeviceManager {
    ...
    private static final String ACTION_USB_PERMISSION = "org.libobject.app.USB_PERMISSION";
    ...
    private HashMap<Integer, org.libobject.player.HIDDevice> mDevicesById = new HashMap<Integer, org.libobject.player.HIDDevice>();
    private HashMap<UsbDevice, org.libobject.player.HIDDeviceUSB> mUSBDevices = new HashMap<UsbDevice, org.libobject.player.HIDDeviceUSB>();
    private HashMap<BluetoothDevice, org.libobject.player.HIDDeviceBLESteamController> mBluetoothDevices = new HashMap<BluetoothDevice, org.libobject.player.HIDDeviceBLESteamController>();
...
//        else
    ...
    private void handleUsbDeviceDetached(UsbDevice usbDevice) {
        org.libobject.player.HIDDeviceUSB device = mUSBDevices.get(usbDevice);
        if (device == null)
            return;
    ...
    private void connectHIDDeviceUSB(UsbDevice usbDevice) {
        synchronized (this) {
            for (int interface_number = 0; interface_number < usbDevice.getInterfaceCount(); interface_number++) {
                if (isHIDDeviceInterface(usbDevice, interface_number)) {
                    org.libobject.player.HIDDeviceUSB device = new org.libobject.player.HIDDeviceUSB(this, usbDevice, interface_number);
                    int id = device.getId();
    ...
    public boolean connectBluetoothDevice(BluetoothDevice bluetoothDevice) {
        ...
        synchronized (this) {
            if (mBluetoothDevices.containsKey(bluetoothDevice)) {
                ...
                org.libobject.player.HIDDeviceBLESteamController device = mBluetoothDevices.get(bluetoothDevice);
                device.reconnect();

                return false;
            }
            org.libobject.player.HIDDeviceBLESteamController device = new org.libobject.player.HIDDeviceBLESteamController(this, bluetoothDevice);
            int id = device.getId();
    ...
    private void close() {
        ...
        shutdownBluetooth();
        synchronized (this) {
            for (org.libobject.player.HIDDevice device : mDevicesById.values()) {
                device.shutdown();
            }
            mDevicesById.clear();
    ...
    private org.libobject.player.HIDDevice getDevice(int id) {
        synchronized (this) {
            org.libobject.player.HIDDevice result = mDevicesById.get(id);
            if (result == null) {
    ...
    public boolean openDevice(int deviceID) {
        ...
        for (org.libobject.player.HIDDeviceUSB device : mUSBDevices.values()) {
        ...
        try {
            ...
            org.libobject.player.HIDDevice device;
