 ...
 * DMA 触发 dma_run：
 *   - dma_run：读取调用方填好的 (dma_src, dma_dst, dma_len) 通过 edu 的
...
static int __edu_dma_irq_handler(void *opaque)
{
    ...
    if (vfio->region_read(vfio, 0, EDU_IRQ_STATUS, &irq_status) == 0 &&
        (irq_status & EDU_DMA_IRQ_BIT)) {
        vfio->region_write(vfio, 0, EDU_IRQ_CLEAR, EDU_DMA_IRQ_BIT);
    }
...
static int __wait_dma_done(Vfio_Pcie_Edu *edu)
{
    ...
    if (((Vfio *)edu)->region_read((Vfio *)edu, 0, EDU_IRQ_STATUS,
                                   &irq_status) == 0) {
...
/*
 * dma_run（override Vfio.dma_run）：执行调用方配置好的两段式内存搬运，
...
static int __dma_run(Vfio_Pcie_Edu *edu)
{
    ...
    if (vfio->region_write(vfio, 0, EDU_IRQ_CLEAR, EDU_DMA_IRQ_BIT) < 0 ||
        vfio->region_write(vfio, 0, EDU_DMA_SRC, pcie->dma_src) < 0 ||
        vfio->region_write(vfio, 0, EDU_DMA_DST, EDU_DMA_START) < 0 ||
        vfio->region_write(vfio, 0, EDU_DMA_CNT,
                           (uint64_t)pcie->dma_len) < 0 ||
        vfio->region_write(vfio, 0, EDU_DMA_CMD,
    ...
    if (vfio->region_write(vfio, 0, EDU_IRQ_CLEAR, EDU_DMA_IRQ_BIT) < 0 ||
        vfio->region_write(vfio, 0, EDU_DMA_SRC, EDU_DMA_START) < 0 ||
        vfio->region_write(vfio, 0, EDU_DMA_DST, pcie->dma_dst) < 0 ||
        vfio->region_write(vfio, 0, EDU_DMA_CNT,
                           (uint64_t)pcie->dma_len) < 0 ||
        vfio->region_write(vfio, 0, EDU_DMA_CMD,
...
static int __construct(Vfio_Pcie_Edu *module, char *init_str)
{
    ...
    /* edu DMA 寄存器是 32 位：配置 region 访问默认位宽（region_read/write 传 0 即用它） */
    vfio->set_width(vfio, 32);
...
/*
 * Vfio_Pcie_Edu 注册接口。
 * 接口继承：dma_run 用实际实现覆盖（设备相关），经 __object_override_virtual_func
 * 自动覆盖基类字段，通过 (Vfio *)edu 或 edu->parent 调用都会派发到本实现。
 */
DEFINE_CLASS(
