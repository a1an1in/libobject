 ...
 * v3 多会话模型下，打洞目标(会话地址)不再在 REG 上报，而是随 INVITE/INVITE_REPLY 信令
 * 交换；因此登记表只记信令地址，用于给该节点投递 INVITE/INVITE_REPLY/CONNECTED。
 */
 ...
 *  - from_host/from_port = 主叫 INVITE 的信令源地址（回执 INVITE_REPLY/CONNECTED 投递）
 *  - to_host/to_port     = 被叫 INVITE_REPLY(accept) 带回的【本会话打洞地址】（转给主叫用）
 * 两端是否打通以各自 PUNCHOK 上报为准（caller_ok/callee_ok 都置位才判定 ok，
 * 回双方 CONNECTED；INVITE_REPLY(accept) 只是"愿意配合打洞"，非成功）。
 */
typedef struct p2p_server_call_s {
    char key[96];        /* "caller_id|callee_id"，map key */
    char from_id[32];    /* 主叫 stun id */
    char from_host[64];  /* 主叫信令地址（服务器回执 INVITE_REPLY/CONNECTED 投递） */
    int  from_port;
    char to_id[32];      /* 被叫 stun id */
    char to_host[64];    /* 被叫本会话打洞地址(INVITE_REPLY(accept) 带回，转给主叫) */
    int  to_port;
    int  from_nat;       /* 主叫 NAT(INVITE 带回，转给被叫) */
    int  to_nat;         /* 被叫 NAT(INVITE_REPLY(accept) 带回，转给主叫) */
 ...
 *  - 信令：SIGNIN/SIGNOUT 登记 + INVITE/INVITE_REPLY/PUNCHOK 撮合（会话地址交换）
 *    信令处理是**表驱动**的（见 P2p_Server.c 的 g_stun_sig_table：关键字 -> 处理函数，
 *    新增信令加一个函数 + 一行表项即可）。
