...
static int test_vector_to_json_case5()
{
    int ret = 0, help = 0;
    allocator_t *allocator = allocator_get_default_instance();
    int value_type = VALUE_TYPE_STRUCT_POINTER;
    ...
    TRY {
        ...
        vector->set(vector, "/Vector/value_type", &value_type);
        vector->set(vector, "/Vector/class_name", "Simplest_Struct_Adapter");
...
static int test_vector_to_json(TEST_ENTRY *entry)
{
    ...
    TRY {
        ...
        EXEC(test_vector_to_json_case5());
    } CATCH (ret) { }
...
static int test_vector_assign_case5()
{
    int ret;
    Vector *vector;
    allocator_t *allocator = allocator_get_default_instance();
