...
static int test_tar_list(TEST_ENTRY *entry, int argc, void **argv)
{
    ...
    Archive *archive;
    Vector *infos;
	char *zip_file = "./tests/archive/res/tar/test_extract.tar";

    TRY {
        dbg_str(DBG_INFO, "test_tar_list");
        fs_mkdir("./tests/archive/output/tar/", 0777);
        archive = object_new(allocator, "Tar", NULL);
        archive->set_extracting_path(archive, "./tests/archive/output/tar/");
        ...
        fs_rmdir("./tests/archive/output/tar/");
    }
...
static int test_tar_extract_file(TEST_ENTRY *entry, int argc, void **argv)
{
    ...
    Vector *infos;
	char *archive_file = "./tests/archive/res/tar/test_extract.tar";
    char *ref_file = "./tests/archive/res/tar/test_extract.txt";
    ...
    TRY {
		...
		EXEC(archive->set_extracting_path(archive, "./tests/archive/output/tar/"));
        ...
        for (i = 0; i < count; i++) {
            ...
            ret = assert_file_equal("./tests/archive/output/tar/test_extract.txt", ref_file);
            THROW_IF(ret == 1, 1);
        }
		THROW(-1);
    } CATCH (ret) { } FINALLY {
...
static int test_tar_extract_all(TEST_ENTRY *entry, int argc, void **argv)
{
    ...
    allocator_t *allocator = allocator_get_default_instance();
    Archive *archive;
	char *src_file = "./tests/archive/res/tar/test_extract.tar";
    char *ref_file = "./tests/archive/res/tar/test_extract.txt";
...
static int test_tar_add_file(TEST_ENTRY *entry, int argc, void **argv)
{
    ...
    Archive *archive;
    archive_file_info_t info;
    char *file1 = "./tests/archive/res/tar/test_gzip.txt";
    char *file2 = "./tests/archive/res/tar/subdir/test.txt";
    char *tar_name = "./tests/archive/output/tar/test_create.tar";
    ...
    TRY {
        ...
        EXEC(archive->set_extracting_path(archive, "./tests/archive/output/tar/"));
        EXEC(archive->set_adding_path(archive, "./tests/archive/res/tar/"));
        ...
        EXEC(archive->extract(archive));
        ret = assert_file_equal("./tests/archive/output/tar/test_gzip.txt", "./tests/archive/res/tar/test_gzip.txt");
        THROW_IF(ret != 1, -1);
    } CATCH (ret) { } FINALLY {
        object_destroy(archive);
        fs_rmdir("./tests/archive/output/tar/");
...
static int test_tar_add_all(TEST_ENTRY *entry, int argc, void **argv)
{
    ...
    allocator_t *allocator = allocator_get_default_instance();
    Archive *archive;
    char *tar_name = "./tests/archive/output/tar/test_add.tar";
    ...
    TRY {
        ...
        EXEC(archive->save(archive));
        EXEC(archive->extract(archive));
        ret = assert_file_equal("./tests/archive/output/tar/test_extract.txt", "./tests/archive/res/tar/test_extract.txt");
        THROW_IF(ret != 1, -1);
    } CATCH (ret) { } FINALLY {
        object_destroy(archive);
        fs_rmdir("./tests/archive/output/tar/");
