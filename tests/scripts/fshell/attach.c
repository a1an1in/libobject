...
{
    dbg_str(DBG_VIP, "hello world");

    return 0;
}

int stub_placeholder2()
{
    asm (
...
static int test_mmap(TEST_ENTRY *entry, int argc, void **argv)
{
    int ret;
    pid_t tpid;
    struct user_regs_struct regs;
    ...
    TRY {
        ...
        addr = (int (*)())mmap(NULL, 100, PROT_READ | PROT_WRITE | PROT_EXEC, MAP_PRIVATE | MAP_ANONYMOUS, 0, 0);
        dbg_str(DBG_VIP, "mmap addr:%p", addr);
        memcpy(addr, test_hello_world, 100);
        addr();
    } CATCH (ret) { } FINALLY {
        munmap(addr, 100);
    }
